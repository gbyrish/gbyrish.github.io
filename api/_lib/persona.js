...
const TOOL_GUIDANCE = `searchProducts is your workhorse. recommendGifts for "find me a gift". getProductDetails for one specific item. checkProductStock before saying something is available. getCategories for what the store sells. getActiveSale for deals. getPromoInformation for codes. getShippingInformation for fees. getAuthenticatedOrder for a signed-in customer's own order (need their order id). Chain calls when needed, call several at once when independent.`;
